...
then	
		#Node Status
			...
			#OUTPUT="$(./monero/build/release/bin/monerod --rpc-bind-ip=${DEVICE_IP} --rpc-bind-port=${MONERO_PORT} --rpc-login=${RPCu}:${RPCp} --rpc-ssl disabled status | sed -n 's/Height:/&/p')" && echo "$OUTPUT" > /var/www/html/Node_Status.txt
				#JSON output
				curl -su ${RPCu}:${RPCp} --digest -X POST http://${DEVICE_IP}:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_info"}' -H 'Content-Type: application/json' | cat -s | jq -Mr {"Monero_Version:.result.version,Node_Status:.result.status,Current_Height:.result.height,Outgoing_Connections:.result.outgoing_connections_count,Incoming_Connections:.result.incoming_connections_count,Network_Type:.result.nettype,TX_Pool_Size:.result.tx_pool_size,White_Peerlist_Size:.result.white_peerlist_size,Grey_Peerlist_Size:.result.grey_peerlist_size,Update_Available:.result.update_available"} | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;s/^$/ /g;' > /var/www/html/Node_Status.txt && echo "Our network peers are reporting a longest chain of:" >> /var/www/html/Node_Status.txt && curl -su ${RPCu}:${RPCp} --digest -X POST http://${DEVICE_IP}:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_count"}' -H 'Content-Type: application/json' |  jq -Mr "{Target_Block_Height:.result.count}" | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;' >> /var/www/html/Node_Status.txt
...
then
		#Adapted command for tor rpc calls (payments) - RPC port and IP fixed due to tor hidden service settings linked in /etc/tor/torrc
			...
			#JSON output
			curl -su ${RPCu}:${RPCp} --digest -X POST http://127.0.0.1:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_info"}' -H 'Content-Type: application/json' | cat -s | jq -Mr {"Monero_Version:.result.version,Node_Status:.result.status,Current_Height:.result.height,Outgoing_Connections:.result.outgoing_connections_count,Incoming_Connections:.result.incoming_connections_count,Network_Type:.result.nettype,TX_Pool_Size:.result.tx_pool_size,White_Peerlist_Size:.result.white_peerlist_size,Grey_Peerlist_Size:.result.grey_peerlist_size,Update_Available:.result.update_available"} | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;s/^$/ /g;' > /var/www/html/Node_Status.txt && echo "Our network peers are reporting a longest chain of:" >> /var/www/html/Node_Status.txt && curl -su ${RPCu}:${RPCp} --digest -X POST http://127.0.0.1:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_count"}' -H 'Content-Type: application/json' |  jq -Mr "{Target_Block_Height:.result.count}" | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;' >> /var/www/html/Node_Status.txt
fi
...
then
		#Adapted command for restricted public rpc calls (payments)
			...
			#JSON output
			curl -su --digest -X POST http://${DEVICE_IP}:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_info"}' -H 'Content-Type: application/json' | cat -s | jq -Mr {"Monero_Version:.result.version,Node_Status:.result.status,Current_Height:.result.height,Outgoing_Connections:.result.outgoing_connections_count,Incoming_Connections:.result.incoming_connections_count,Network_Type:.result.nettype,TX_Pool_Size:.result.tx_pool_size,White_Peerlist_Size:.result.white_peerlist_size,Grey_Peerlist_Size:.result.grey_peerlist_size,Update_Available:.result.update_available"} | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;s/^$/ /g;' > /var/www/html/Node_Status.txt && echo "Our network peers are reporting a longest chain of:" >> /var/www/html/Node_Status.txt && curl -su --digest -X POST http://${DEVICE_IP}:${MONERO_PORT}/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_count"}' -H 'Content-Type: application/json' |  jq -Mr "{Target_Block_Height:.result.count}" | sed 's/"//g;s/{//g;s/}//g;s/,//g;s/_/ /g;' >> /var/www/html/Node_Status.txt
